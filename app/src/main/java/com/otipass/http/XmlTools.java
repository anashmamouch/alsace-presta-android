...
================================================================================
    ...
    @version $Rev: 4152 $
    $Id: XmlTools.java 4152 2014-09-11 12:21:57Z ede $
 ...
 */package com.otipass.http;
 ...
 public class XmlTools {
	 ...
	 private Otipass otipassChecked;
	 private int dbModel;
	 ...
	 private static final String TAG_FARE_ID = "fid";
	 private static final String TAG_OPTION = "option";
	 private static final String TAG_OPTION_ID = "oid";
	 private static final String TAG_BDD_MODEL = "db_model";
	 private static final String TAG_PACKAGE_FARE = "package_fare";
	 // these tags added because server is sending the DB fields tags
	 private static final String TAG_PACKAGE_FARE_PID = "package_id";
	 private static final String TAG_PACKAGE_FARE_FID = "fare_id";
	 private static final String TAG_PACKAGE_FARE_OID = "option_id";
	 ...
	 public  int decodeJSONResponse(InputStream inputStream) {
		 int status = SynchronizationService.cComClientBadRequest;
		 try {
			 ...
			 String sXml = XML.toString(json);

			 DocumentBuilderFactory documentBuilderFactory = DocumentBuilderFactory.newInstance();
			 DocumentBuilder documentBuilder = documentBuilderFactory.newDocumentBuilder();
			 dom = documentBuilder.parse(new InputSource(new StringReader(sXml)));
	 ...
	 public String inputStr2String(InputStream in) throws IOException {
		 ...
		 try {
			 ...
			 if (nodes.getLength() > 0) {
				 ...
				 for (int i=0; i<nodes.getLength(); i++) {
					 ...
					 try {
						 fare = Integer.valueOf(ParseElement((Element)node, TAG_FARE_ID));
					 }catch (Exception ex) {
						 fare = -1;
					 };
					 try {
						 option = Integer.valueOf(ParseElement((Element)node, TAG_OPTION_ID));
					 }catch (Exception ex) {
						 option = 0;
					 };
					 otipassList.add(new Otipass(numOtipass, serial, cardStatus, expiry, type, pid, service, fare, option));
					 if (day.length() > 0) {
	 ...
	 public  int decodeWLPartial() {
		 int status = SynchronizationService.cComClientDecodeFailure;
		 int version, numotipass, type, day = -1, pid = 1, option=-1, fare=-1;
		 short cardStatus;
			 ...
			 if (status == SynchronizationService.cComOK){
				 ...
				 if (nodes.getLength() > 0) {
					 ...
					 for (int i=0; i<nodes.getLength(); i++) {
						 ...
						 if (ParseElement((Element)node, TAG_FARE_ID).length() > 0) {
							 fare = Integer.valueOf(ParseElement((Element)node, TAG_FARE_ID));
							 if (ParseElement((Element)node, TAG_OPTION_ID).length() > 0) {
								 option = Integer.valueOf(ParseElement((Element)node, TAG_OPTION_ID));
							 }
						 }
						 partialList.add(new Partial(numotipass, cardStatus, expiry, pid, fare, option));
						 Log.d("PARTIAL", "partialList.add " + ParseElement((Element)node, TAG_NUMOTIPASS) + " - " + ParseElement((Element)node, TAG_STATUS) + " - " + ParseElement((Element)node, TAG_EXPIRY) + " - " + ParseElement((Element)node, TAG_PID)
								 + " - " + ParseElement((Element)node, TAG_FARE_ID) + " - " + ParseElement((Element)node, TAG_OPTION_ID));
					 }
				 }
			 }
			 if (status == SynchronizationService.cComOK){
			 ...
			 if (status == SynchronizationService.cComOK){
				 ...
				 if (nodes.getLength() > 0) {
					 ...
					 for (int i=0; i < nodes.getLength(); i++) {
						 ...
						 createList.add(new Create(numotipass, (short)type, serial, cardStatus, pid, srv, "", fare, option));
					 }
				 }
			 }
		 } catch (Exception e) {
	 ...
	 public  int decodeParam() {
		 int status = SynchronizationService.cComClientDecodeFailure;
		 int debug, id, idpackage, idservice, type, number, category, nbChild, duration, period;
		 ...
		 try {
			 ...
			 if (nodes.getLength() > 0) {
				 ...
				 try {
					 dbModel = Integer.valueOf(ParseElement((Element) node, TAG_BDD_MODEL));
				 } catch (Exception ex) {
					 dbModel = 1;
				 };
			 }
			 ...
			 if (nodesPackage.getLength() > 0) {
				 for (int i=0; i<nodesPackage.getLength(); i++) {
					 ...
					 try {
						 nbChild = Integer.valueOf(ParseElement((Element) node, TAG_CHILD));
					 } catch (Exception ex) {nbChild = 0;}
					 ...
					 try {
						 ...
						 packageList.add(new PackageObject(idpackage, package_name, nbChild, duration, period, price, ref));
					 } catch (Exception ex) {
						 packageList.add(new PackageObject(idpackage, package_name, nbChild, duration, period, ref));
					 }
			 ...
			 nodes = dom.getElementsByTagName(TAG_FARE);
			 if (nodes.getLength() > 0) {
				 for (int j=0; j<nodes.getLength(); j++) {
					 Node node = nodes.item(j);
					 ...
					 name = ParseElement((Element)node, TAG_NAME);
					 ref = ParseElement((Element)node, TAG_REFERENCE);
					 if ((id > 0) && (name.length() > 0) && (ref.length() > 0)) {
						 fareList.add(new Fare(id, name, ref));
					 }
				 }
			 }
			 optionList = new ArrayList<Option>();
			 nodes = dom.getElementsByTagName(TAG_OPTION);
			 if (nodes.getLength() > 0) {
				 for (int j=0; j<nodes.getLength(); j++) {
					 Node node = nodes.item(j);
					 id = Integer.valueOf(ParseElement((Element)node, TAG_ID));
					 ...
					 ref = ParseElement((Element)node, TAG_REFERENCE);
					 if ((id > 0) && (name.length() > 0) && (ref.length() > 0)) {
						 optionList.add(new Option(id, name, ref));
					 }
				 }
			 }
			 packageFareList = new ArrayList<PackageFare>();
			 nodes = dom.getElementsByTagName(TAG_PACKAGE_FARE);
			 if (nodes.getLength() > 0) {
				 for (int j=0; j<nodes.getLength(); j++) {
					 Node node = nodes.item(j);
					 idpackage = Integer.valueOf(ParseElement((Element)node, TAG_PACKAGE_FARE_PID));
					 int idfare = Integer.valueOf(ParseElement((Element)node, TAG_PACKAGE_FARE_FID));
					 int idoption = Integer.valueOf(ParseElement((Element)node, TAG_PACKAGE_FARE_OID));
					 price = Double.valueOf(ParseElement((Element)node, TAG_PRICE));
					 if (idpackage > 0) {
						 packageFareList.add(new PackageFare(idpackage, idfare, idoption, price));
					 }
				 }
			 }
	 ...
	 public  Otipass decodeCheckNumOtipass() {
		 int status = SynchronizationService.cComClientDecodeFailure;
		 int idotipass = 0, pid = 1, fare_id = -1, option_id = -1;
		 String expiryDate = "", service = "", day = "", serial="";
		 long numotipass = 0;
		 short st = 0, type = 0;
		 try {
			 NodeList nodes = dom.getElementsByTagName(TAG_CARD);
			 if (nodes.getLength() > 0) {
				 ...
				 if (node.getFirstChild() != null) {
					 ...
					 try {
						 fare_id = Integer.valueOf(ParseElement((Element) node, TAG_FARE_ID));
					 } catch (Exception ex) {
						 fare_id = -1;
					 }
					 try {
						 option_id = Integer.valueOf(ParseElement((Element)node, TAG_OPTION_ID));
					 } catch (Exception ex) {
						 option_id = -1;
					 }
					 ...
					 otipassChecked = new Otipass(numotipass, serial, st, expiryDate, type, pid, service, day, fare_id, option_id);
				 }
				 status = SynchronizationService.cComOK;
			 } 
		 } catch (Exception e) {
		 ...
		 try {
			 ...
			 if (nodes.getLength() > 0) {
				 ...
				 for (int i=0; i<nodes.getLength(); i++) {
					 ...
					 messageList.add(new Msg(text, startDate, endDate, lang));
				 }
				 status = SynchronizationService.cComOK;
			 } else {
				 status = SynchronizationService.cComOK;
	 ...
	 public  int decodeFare() {
		 int status = SynchronizationService.cComClientDecodeFailure;
		 int id;
		 String name, reference;
		 try {
			 NodeList nodes = dom.getElementsByTagName(TAG_FARE);
			 if (nodes.getLength() > 0) {
				 Node node;
				 status = SynchronizationService.cComOK;
				 fareList = new ArrayList<Fare>();
				 for (int i=0; i<nodes.getLength(); i++) {
					 ...
					 id = Integer.valueOf(ParseElement((Element)node, TAG_ID));
					 name = ParseElement((Element)node, TAG_NAME);
					 reference = ParseElement((Element)node, TAG_REFERENCE);
					 fareList.add(new Fare(id, name, reference));
				 }
			 }
		 } catch (Exception e) {
			 Log.e(TAG, XmlTools.class.getName() + " - decodeFare - " + e.getMessage());
		 }
		 return status;
	 }
	 ...
	 public  int decodeOption() {
		 ...
		 String name, reference;
		 try {
			 NodeList nodes = dom.getElementsByTagName(TAG_OPTION);
			 if (nodes.getLength() > 0) {
				 Node node;
				 status = SynchronizationService.cComOK;
				 optionList = new ArrayList<Option>();
				 for (int i=0; i<nodes.getLength(); i++) {
					 ...
					 name = ParseElement((Element)node, TAG_NAME);
					 reference = ParseElement((Element)node, TAG_REFERENCE);
					 optionList.add(new Option(id, name, reference));
				 }
			 }
		 } catch (Exception e) {
			 Log.e(TAG, XmlTools.class.getName() + " - decodeOption - " + e.getMessage());
		 }
		 return status;
	 }

	 public  int decodePackageFare() {
		 int status = SynchronizationService.cComClientDecodeFailure;
		 int package_id, fare_id, option_id;
		 Double price;
		 try {
			 NodeList nodes = dom.getElementsByTagName(TAG_PACKAGE_FARE);
			 if (nodes.getLength() > 0) {
				 Node node;
				 status = SynchronizationService.cComOK;
				 packageFareList = new ArrayList<PackageFare>();
				 for (int i=0; i<nodes.getLength(); i++) {
					 node = nodes.item(i);
					 package_id = Integer.valueOf(ParseElement((Element)node, TAG_PID));
					 fare_id = Integer.valueOf(ParseElement((Element)node, TAG_FARE_ID));
					 option_id = Integer.valueOf(ParseElement((Element)node, TAG_OPTION_ID));
					 price = Double.valueOf(ParseElement((Element)node, TAG_PRICE));
					 packageFareList.add(new PackageFare(package_id, fare_id, option_id, price));
				 }
			 }
		 } catch (Exception e) {
			 Log.e(TAG, XmlTools.class.getName() + " - decodePackageFare - " + e.getMessage());
		 }
		 return status;
	 }
	 
	 @SuppressLint("NewApi")
	 public String  buildUploadXml(List<Update> updateList) {
		 Update update;
		 ...
		 try {
			 ...
			 serializer.text(android.os.Build.SERIAL);
			 ...
			 if (updateList.size() > 0) {
				 ...
				 for (int i=0; i<updateList.size(); i++) {
					 ...
					 if (update.getFareId() > -1) {
						 serializer.startTag(nameSpace, TAG_FARE_ID);
						 serializer.text(String.valueOf(update.getFareId()));
						 serializer.endTag(nameSpace, TAG_FARE_ID);
						 serializer.startTag(nameSpace, TAG_OPTION_ID);
						 serializer.text(String.valueOf(update.getOptionId()));
						 serializer.endTag(nameSpace, TAG_OPTION_ID);
					 }
					 ...
					 if ((update.getPostalCode() != null)&& !update.getPostalCode().isEmpty()) {
	 ...
	 @SuppressLint("NewApi")
	 public String  buildEntryXml(List<Entry> entryList) {
		 Entry entry;
		 String xmlString = "", nameSpace = "";
		 XmlSerializer serializer = Xml.newSerializer();
		 StringWriter writer = new StringWriter();
