...
public class NavDrawerListAdapter extends BaseAdapter {
	...
	public void checkItem(int position, int lastPosition) {
		selectedItem = position;
		lastSelectedItem = lastPosition;
		indexItem = 0;
		...
		for(Object section : this.sections.keySet()) {
			...
			if(position < size) {
				...
				if (view != null) {
					...
					if(tv != null) {
						if (indexItem == selectedItem) {
							tv.setBackgroundColor(Color.parseColor("#FF9900"));
						} else {
							tv.setBackgroundDrawable(context.getResources().getDrawable(R.drawable.list_selector));
						}
					}
				}
				indexItem++;
