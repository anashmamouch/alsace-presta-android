...
================================================================================
    ...
    Package com.otipass.adapter

    @copyright Otipass 2014. All rights reserved.
    @author  AD

    @version $Rev: 4251 $
    $Id: Nfc.java 4251 2014-10-22 15:37:38Z ado $
...
public class Nfc {
	...
	public static OtipassCard readCard(Intent intent, Bundle extras) {
		...
		try {
			...
			Log.i(Constants.TAG, tools.formatNow(Constants.SQL_FULL_DATE_FORMAT)+" NFC detect:"+pupi+" - "+message);
			if (!pupi.isEmpty()) {
				if ((message != null) && !message.isEmpty()) {
					tags = message.split("-");
					if (tags.length == 2) {
//						client = tags[0];
						client = Constants.TAG;
						...
						Log.i(Constants.TAG, "NFC card detected:"+numOtipass);
					}
				}
			}
		} catch (Exception e) {
